...
Процедура ВыбратьПериод(Команда)
	...
	СтандартныеОтчетыКлиент.ВыбратьПроизвольныйПериодОтчета(ЭтаФорма, Отчет.НачалоПериода, Отчет.КонецПериода, ДоступныеПериодыОтчета.День);
...
Процедура СформироватьОтчетНаСервере()
	...
	НоваяДата = Отчет.НачалоПериода;
	Пока НоваяДата <= Отчет.КонецПериода Цикл
		СписокПериодов.Добавить(НоваяДата);
		НоваяДата = НоваяДата + 86400;
	КонецЦикла;	
	
	ТабличныйДокумент = ЭтаФорма.Результат;
	...
	ИначеЕсли Вариант = "СравнениеСГрафиком" Тогда
		
		ОбластьМакета = Макет.ПолучитьОбласть("ШапкаТаблицы|ИтогоУУ");
		ТабличныйДокумент.Присоединить(ОбластьМакета);
		
		ОбластьМакета = Макет.ПолучитьОбласть("ШапкаТаблицы|Норма");
		ТабличныйДокумент.Присоединить(ОбластьМакета);
		
		ОбластьМакета = Макет.ПолучитьОбласть("ШапкаТаблицы|ДельтаВЧасах");
		ТабличныйДокумент.Присоединить(ОбластьМакета)
		
	КонецЕсли;
	
	Для Каждого ПараметрДанныхЭлемент Из Отчет.КомпоновщикНастроек.ФиксированныеНастройки.ПараметрыДанных.Элементы Цикл
		
		Если ПараметрДанныхЭлемент.Параметр = Новый ПараметрКомпоновкиДанных("НачалоПериода") Тогда
			ПараметрДанныхЭлемент.Значение = Отчет.НачалоПериода;
			ПараметрДанныхЭлемент.Использование = Истина;
		ИначеЕсли ПараметрДанныхЭлемент.Параметр = Новый ПараметрКомпоновкиДанных("ОкончаниеПериода") Тогда
			ПараметрДанныхЭлемент.Значение = Отчет.КонецПериода;
			ПараметрДанныхЭлемент.Использование = Истина;	
		ИначеЕсли ПараметрДанныхЭлемент.Параметр = Новый ПараметрКомпоновкиДанных("ПолныеПрава") Тогда
			ПараметрДанныхЭлемент.Значение = РольДоступна("ПолныеПрава");
			ПараметрДанныхЭлемент.Использование = Истина;
		ИначеЕсли ПараметрДанныхЭлемент.Параметр = Новый ПараметрКомпоновкиДанных("РазрешенныеЦФО") Тогда
			...
			ПараметрДанныхЭлемент.Значение = РазрешенныеЦФО;
			ПараметрДанныхЭлемент.Использование = Истина;
		КонецЕсли;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаВывода Цикл
		...
		Если СтрокаТаблицы.Показатель = "График" Тогда
			...
			Если ЗначениеЗаполнено(СтрокаТаблицыОтчета[ОбщееИмя + "Начало_График"]) И СтрокаТаблицы.НачалоДня <> Дата('00010101')  Тогда
				СтрокаТаблицыОтчета[ОбщееИмя + "Начало_График"] = Мин(СтрокаТаблицыОтчета[ОбщееИмя + "Начало_График"], СтрокаТаблицы.НачалоДня);
			ИначеЕсли СтрокаТаблицы.НачалоДня <> Дата('00010101') Тогда	
	...
	Для Каждого СтрокаТаблицы Из ТаблицаОтчета Цикл
		...
		Для Итерация = 1 По КоличествоИтераций Цикл
			...
			Если Итерация = 2 Тогда
				ОбъединяемаяОбласть = ТабличныйДокумент.Область(ТабличныйДокумент.ВысотаТаблицы - 1, 1, ТабличныйДокумент.ВысотаТаблицы, 2);
			...
			Для Каждого ЭлементСписка Из СписокПериодов Цикл
				...
				Если Вариант = "СравнениеСГрафиком" Тогда
					Если Итерация = 1 Тогда
						ОбластьМакета.Параметры.ТекстДляВывода = Формат((СтрокаТаблицы[ОбщееИмя+"Окончание_График"] - СтрокаТаблицы[ОбщееИмя+"Начало_График"])/3600, "ЧДЦ=2; ЧГ=");
					ИначеЕсли Итерация = 2 Тогда
						ОбластьМакета.Параметры.ТекстДляВывода = Формат((СтрокаТаблицы[ОбщееИмя+"Окончание_Проходная"] - СтрокаТаблицы[ОбщееИмя+"Начало_Проходная"])/3600, "ЧДЦ=2; ЧГ=");		
					КонецЕсли;
				КонецЕсли;	
			...
			ИначеЕсли Вариант = "СравнениеСГрафиком" Тогда
				...
				ОбластьМакета = Макет.ПолучитьОбласть("Строка0|ИтогоУУ");
				Если Итерация = 1 Тогда
					ОбластьМакета.Параметры.ИтогоУУ = Формат(ОбщееВремяРаботыГ/3600, "ЧДЦ=2; ЧГ=");
				ИначеЕсли Итерация = 2 Тогда
					ОбластьМакета.Параметры.ИтогоУУ = Формат(ОбщееВремяРаботыФ/3600, "ЧДЦ=2; ЧГ=");
				КонецЕсли;
				ТабличныйДокумент.Присоединить(ОбластьМакета);
				...
				ОбластьМакета = Макет.ПолучитьОбласть("Строка0|ДельтаВЧасах");
				Если Итерация = 1 Тогда
					ОбластьМакета.Параметры.ДельтаВЧасах = Формат(ОбщееВремяРаботыГ/3600 - Норма, "ЧДЦ=2; ЧГ=");
				ИначеЕсли Итерация = 2 Тогда
					ОбластьМакета.Параметры.ДельтаВЧасах = Формат(ОбщееВремяРаботыФ/3600 - Норма, "ЧДЦ=2; ЧГ=");;
				КонецЕсли;
				ТабличныйДокумент.Присоединить(ОбластьМакета);
