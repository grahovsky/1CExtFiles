...
Процедура ВыполнитьHTTPЗапросНаСервере(СтруктураЗаявки)
	...
	ТехническийКлюч = "&TECHNICIAN_KEY=279B0608-A2EF-4458-805B-92CA12DC8F75";
...
Процедура ПодготовитьЗаявкиИПисьма()
	...
	МассивРаботников.Добавить(СсылкаНаОбъект);
	Попытка
		ТаблицаТ5 = КадровыйУчетБазовый.СформироватьЗапросДляТ5(МассивРаботников).Выгрузить();
	Исключение
	КонецПопытки;
	Если ТаблицаТ5 = Неопределено ИЛИ ТаблицаТ5.Количество() = 0 Тогда
		ДанныеОПереводе = Неопределено;
	Иначе
		ДанныеОПереводе = ТаблицаТ5.Найти(СсылкаНаОбъект, "Сотрудник");
	КонецЕсли;
...
Функция СформироватьТекстСообщения(ИмяГруппы, СтруктураДанных)
	...
	ИначеЕсли ВидЗаявки = "Перевод" Тогда
		...
		Если ИмяГруппы = "Основная" Или ИмяГруппы = "Техподдержка" Тогда
			...
			"ДатаПеревода: "+Формат(ДанныеОПереводе.Дата,  "ДЛФ=ДД") + Символы.ПС +
			"Из подразделения: " + ДанныеОПереводе.Подразделение + Символы.ПС +
			"В Подразделение: " + ДанныеОПереводе.НовоеПодразделение + Символы.ПС +
			"Юридическое лицо: " + ДанныеОПереводе.НовоеПодразделение.Владелец + Символы.ПС + 
			"ФИО сотрудника на русском: " + ДанныеОПереводе.Работник + Символы.ПС + 
			"Табельный номер: " + ДанныеОПереводе.ТабельныйНомер + Символы.ПС + 
			"С должности: " + ДанныеОПереводе.Должность + Символы.ПС + 
			"На должность: " + ДанныеОПереводе.НоваяДолжность + Символы.ПС + 
			"Права доступа в 1с БП: " + СписокКонтактнойИнформации["Доступ к 1С Бухгалтерия"] + Символы.ПС + 
			"Права доступа в 1с ЗУП: " + СписокКонтактнойИнформации["Доступ к 1С ЗУП"] + Символы.ПС + 
			"Права доступа в 1с УУ: " + СписокКонтактнойИнформации["Доступ к 1С УУ"] + Символы.ПС + 
			"Права доступа в 1с Документооборот: " + СписокКонтактнойИнформации["Доступ к 1С Документооборот"] + Символы.ПС +
		...
		ИначеЕсли ИмяГруппы = "ИТО" Тогда
			...
			"Табельный номер: " + ДанныеОПереводе.ТабельныйНомер + Символы.ПС + 
			"С должности: " + ДанныеОПереводе.Должность + Символы.ПС + 
			"На должность: " + ДанныеОПереводе.НоваяДолжность; 
...
Процедура ЗаполнитьПараметрыРассылки(Группа, СтруктураДанных)
	...
	ИнтернетПочтовоеСообщение.Тема = СформироватьТекстТемы(СтруктураДанных);
...
//	СтруктураДетали.requester = "Князева Зоя Васильевна"; //АвторЗаявки
//	СтруктураДетали.createdby = "Князева Зоя Васильевна"; //КемСоздан
...
Процедура ОтправитьПисьма()
	...
	Для Каждого ГруппаУчастников Из ГруппыУчастников Цикл
		Если Отладка Тогда
			...
			ГруппаУчастников.Значение.ИнтернетПочтовоеСообщение.Получатели.Добавить("kgrahovsky@sogaz-clinic.ru");
		КонецЕсли;
...
Процедура ДобавитьПолучателейОтправителя(ИнтернетПочтовоеСообщение, ИмяГруппы)
	...
	ИначеЕсли ИмяГруппы = "Техподдержка" Тогда
		...
		ИнтернетПочтовоеСообщение.Получатели.Добавить("ekozlova@sogaz-clinic.ru");
