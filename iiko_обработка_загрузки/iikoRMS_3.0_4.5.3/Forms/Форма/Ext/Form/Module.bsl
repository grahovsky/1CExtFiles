...
ТаблицаЗапроса, режим, Слово, ОсновнаяНоменклатурнаяГруппа, Юрлицо, ТекущаяНастройка, НужноеЮрЛицо, 
...
Функция НайтиОгранизациюПоИНН2(ИНН)
	...
	Иначе
		ТестЗапрос = Новый Запрос();
		// + Граховский К.Н. 2018.12.12
		//ТестЗапрос.Текст = "ВЫБРАТЬ Организация ИЗ РегистрСведений.КодыОрганизации ГДЕ ИНН = """ + ИНН + """";
		ТестЗапрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ Организация ИЗ РегистрСведений.КодыОрганизации ГДЕ ИНН = """ + ИНН + """";
		// - Граховский К.Н. 2018.12.12
...
Процедура КонтрагентыЗаполнить(Команда)
	...
	// + Граховский К.Н. 2018.11.12
	// создание контрагентов - всегда закрыто
...
Процедура НоменклатураСоздать(Команда)
	...
	Если Ном <> Неопределено Тогда
		...
		// + Граховский К.Н. 2018.11.09
		// не показываем принудительно
...
//	СТрокаНастроек = XMLСтрока(ТекущаяНастройка);
	...
	// + Граховский К.Н. 2018.11.12
	ДополнительнаяОбработкаПриСозданииНаСервере()
	// - Граховский К.Н. 2018.11.12
...
Функция СоздатьНоменклатуру(Код, Наименование, Категория = Неопределено, СтавкаНДСстр = Неопределено)
	...
	// + Граховский К.Н. 2019.01.22
	//Ном.СтавкаНДС = СтавкаНДС;
	Ном.ВидСтавкиНДС = Перечисления.ВидыСтавокНДС.ВидСтавки(СтавкаНДС);
	// - Граховский К.Н. 2019.01.22
	...
	// + Граховский К.Н. 2018.11.09
	Ном.ВидНоменклатуры = катег[0].Группа1С.ВидНоменклатуры;
	// - Граховский К.Н. 2018.11.09
	
	Попытка
		Ном.Записать();
		// + Граховский К.Н. 2018.11.08
		РегистрыСведений._СоответствияОбъектовАйко.ДобавитьСоответствие("Номенклатура", Код, Ном.Ссылка, Наименование);
		// - Граховский К.Н. 2018.11.08
	Исключение
...
Процедура ОбновитьГрупперовку()
	...
	Элементы.ГруппаГруппировка.Видимость = ЕстьГрупперовка;
...
Процедура ЗагрузитьНаСервере()
	...
	// + Граховский К.Н. 2018.11.09
	ЗаполнитьСоответствияПоРегистру();
	// - Граховский К.Н. 2018.11.09
	...
	Для каждого докСтр из Доки2 цикл
		...
		Если СписДок.Количество() > 1 Тогда
			...
			Для каждого СтрокаМассива из СписДок цикл
				...
				// + Граховский К.Н. 2018.11.09
				Док = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ПоступлениеТоваровУслуг", СтрокаМассива.Номер); 
				// - Граховский К.Н. 2018.11.09
				...
				КонецЕсли;
				Док.Комментарий = "Загружено из iiko " + НомерДокумента;
				// - Граховский К.Н. 2018.11.08
				...
				Для каждого стрр из Табчасть Цикл
					...
					// + Граховский К.Н. 2018.11.12
					ЗаполнитьСпособУчетаНДСДляСтрокиТабличнойЧасти(Стр2, Док);
					// - Граховский К.Н. 2018.11.12
				...
				Попытка
					...
					// + Граховский К.Н. 2018.11.08
					РегистрыСведений._СоответствияОбъектовАйко.ДобавитьСоответствие("ПоступлениеТоваровУслуг", СтрокаМассива.Номер, Док.Ссылка, СтрокаМассива.Номер + " от " + СтрокаМассива.Дата);
					// - Граховский К.Н. 2018.11.08
			...
			Если МассивНаСФ.Количество() > 0 тогда
				...
				Попытка
					...
					// + Граховский К.Н. 2018.11.08
					РегистрыСведений._СоответствияОбъектовАйко.ДобавитьСоответствие("СчетФактураПолученный", СтрокаМассива.СчетФактура, Док1.Ссылка, СтрокаМассива.СчетФактура + " от " + СтрокаМассива.ВходДата);
					// - Граховский К.Н. 2018.11.08
		...
		Иначе
			...
			// + Граховский К.Н. 2018.11.09
			Док = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ПоступлениеТоваровУслуг", СписДок[0].Номер); 
			// - Граховский К.Н. 2018.11.09
			Если не ЗначениеЗаполнено(Док) Тогда
				Док = Документы.ПоступлениеТоваровУслуг.СоздатьДокумент();
				// + Граховский К.Н. 2018.11.08
			...
			Попытка
					...
					// + Граховский К.Н. 2018.11.08
					РегистрыСведений._СоответствияОбъектовАйко.ДобавитьСоответствие("ПоступлениеТоваровУслуг", СписДок[0].Номер, Док.Ссылка, СписДок[0].Номер + " от " + СписДок[0].Дата);
					// - Граховский К.Н. 2018.11.08
...
Процедура СкладыЗаполнить(Команда)
	...
	// + Граховский К.Н. 2018.11.12
	// запрет создания складов и подразделений
...
Процедура СоздатьПриготовленияПодговленные(таб,префикс,ДеньПрефикс,порядок, сдвинутьДату,базовыйДокумент)
	...
	Для каждого Стр из Доки цикл
		...
		// + Граховский К.Н. 2018.11.09
		Док = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ОтчетПроизводстваЗаСмену", Стр.Номер); 
		// - Граховский К.Н. 2018.11.09
		Если не ЗначениеЗаполнено(Док) Тогда
		...
		Если Док.ЭтоНовый() Тогда
			Док.УстановитьНовыйНомер();
		КонецЕсли;
		Док.Комментарий = "Загружено из iiko " + Стр.Номер;
		// - Граховский К.Н. 2018.11.09
		...
		Если Не Проводить тогда 
			Попытка 
				...
				КонецЕсли;
				// + Граховский К.Н. 2018.11.08
				РегистрыСведений._СоответствияОбъектовАйко.ДобавитьСоответствие("ОтчетПроизводстваЗаСмену", Стр.Номер, Док.Ссылка, Стр.Номер + " от " + Стр.Дата);
				// - Граховский К.Н. 2018.11.08
			Исключение
...
Функция ПолучитьСчетУчетаРасчетовСКонтрагентами(контрагент)
	...
	ТестЗапрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ * ИЗ РегистрСведений.СчетаУчетаРасчетовСКонтрагентами ГДЕ Контрагент = (&Контрагент1)";
	// - Граховский К.Н. 2018.12.12
...
Процедура СоздатьПеремещения(табз,префикс,ДеньПрефикс,порядок)
	...
	Для каждого докСтр из Доки цикл
		...
		// + Граховский К.Н. 2018.11.12
		Док = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ПеремещениеТоваров", докСтр.Номер); 
		// - Граховский К.Н. 2018.11.12
		Если не ЗначениеЗаполнено(Док) Тогда
		...
		Если Док.ЭтоНовый() Тогда
			Док.УстановитьНовыйНомер();
		КонецЕсли;
		Док.Комментарий = "Загружено из iiko " + докСтр.Номер;
		// - Граховский К.Н. 2018.11.12
		...
		// + Граховский К.Н. 2019.02.14
		Док.Товары.Сортировать("Номенклатура Возр");
		// - Граховский К.Н. 2019.02.14
		...
		Если Проводить тогда 			
			...
			Исключение
				...
				Док.Записать(РежимЗаписиДокумента.Запись);
				// + Граховский К.Н. 2018.11.12
				РегистрыСведений._СоответствияОбъектовАйко.ДобавитьСоответствие("ПеремещениеТоваров", докСтр.Номер, Док.Ссылка, докСтр.Номер + " от " + докСтр.Дата);
				// - Граховский К.Н. 2018.11.12
			КонецПопытки;
		Иначе
			Попытка 
		...
		// + Граховский К.Н. 2019.02.14
		СоздатьТребованиеНакладные(Док.Ссылка, докСтр);
		// - Граховский К.Н. 2019.02.14
...
// + Граховский К.Н. 2019.02.14
&НаСервере
Процедура СоздатьТребованиеНакладные(ПеремещениеСсылка, ДанныеСтроки)

	Продолжать = Ложь;
	
	Для Каждого СтрокаТаблицыСкладов Из ТаблицаСкладовДляТребованийНакладных Цикл
		
		Если СтрокаТаблицыСкладов.СкладОтправитель = ПеремещениеСсылка.СкладОтправитель
			И СтрокаТаблицыСкладов.СкладПолучатель = ПеремещениеСсылка.СкладПолучатель 
			И СтрокаТаблицыСкладов.Использование Тогда
			Продолжать = Истина;
		КонецЕсли;
	...
	ТребованиеНакладнаяСсылка = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ТребованиеНакладная", ДанныеСтроки.Номер);
	
	ТребованиеНакладнаяОбъект = Документы.ТребованиеНакладная.СоздатьДокумент();
	
	Если ТребованиеНакладнаяСсылка <>  Неопределено Тогда
		ТребованиеНакладнаяОбъект = ТребованиеНакладнаяСсылка.ПолучитьОбъект();
	Иначе
		ТребованиеНакладнаяОбъект = Документы.ТребованиеНакладная.СоздатьДокумент();
	КонецЕсли;
	
	ТребованиеНакладнаяОбъект.Заполнить(ПеремещениеСсылка);
	ТребованиеНакладнаяОбъект.Дата = ПеремещениеСсылка.Дата + 300;
	ТребованиеНакладнаяОбъект.Субконто1 = СтатьяЗатрат;
	ТребованиеНакладнаяОбъект.Комментарий = ПеремещениеСсылка.Комментарий;
	ТребованиеНакладнаяОбъект.Записать();
	ТребованиеНакладнаяОбъектСсылка = ТребованиеНакладнаяОбъект.Ссылка;
	
	РегистрыСведений._СоответствияОбъектовАйко.ДобавитьСоответствие("ТребованиеНакладная", ДанныеСтроки.Номер, ТребованиеНакладнаяОбъектСсылка, ДанныеСтроки.Номер + " от " + ДанныеСтроки.Дата);
	
	СтрокиДокументы = Объект.Докум.НайтиСтроки(Новый Структура("НомерДок", ДанныеСтроки.Номер));
	Если СтрокиДокументы.Количество() > 0 Тогда
		СтрокиДокументы[0].Док2 = ТребованиеНакладнаяОбъектСсылка;
	КонецЕсли;

КонецПроцедуры // СоздатьТребованиеНакладные()
// - Граховский К.Н. 2019.02.14
...
Процедура СоздатьАктыРеализации(табз,префикс,ДеньПрефикс,порядок)
	...
	Для каждого Стр из Доки цикл
		...
		// + Граховский К.Н. 2018.11.09
		Док = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ОтчетОРозничныхПродажах", Стр.Номер); 
		// - Граховский К.Н. 2018.11.09
		Если не ЗначениеЗаполнено(Док) Тогда
		...
		// + Граховский К.Н. 2019.01.25
		Если ЗначениеЗаполнено(Док.Склад) Тогда
			Док.ТипЦен = Док.Склад.ТипЦенРозничнойТорговли;
		КонецЕсли;
		// - Граховский К.Н. 2019.01.25
		...
		// + Граховский К.Н. 2018.11.08
		//Док.Номер = НомерДокумента;
		НайденноеПодразделение = Объект.Склады.НайтиСтроки(Новый Структура("Склад",Док.Склад))[0].Подразделение;
		Если Не ЗначениеЗаполнено(НайденноеПодразделение) Тогда
			НайденноеПодразделение = Подразделение;	
		КонецЕсли;
		Док.ПодразделениеОрганизации = НайденноеПодразделение;
		...
		Если Проводить тогда 
			...
			КонецПопытки;
			// + Граховский К.Н. 2018.11.08
			РегистрыСведений._СоответствияОбъектовАйко.ДобавитьСоответствие("ОтчетОРозничныхПродажах", Стр.Номер, Док.Ссылка, Стр.Номер + " от " + Стр.Дата);
			// - Граховский К.Н. 2018.11.08
		Иначе
			Попытка 
				Док.Записать(РежимЗаписиДокумента.Запись);  
...
Процедура СоздатьАктыСписания(строки,префикс,порядок)
	...
	Для каждого докСтр из Доки цикл
		...
		// + Граховский К.Н. 2018.11.09
		Док = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ТребованиеНакладная", докСтр.Номер); 
		// - Граховский К.Н. 2018.11.09
		Если не ЗначениеЗаполнено(Док) Тогда
		...
		Для каждого стрр из Табчасть Цикл
			...
			// + Граховский К.Н. 2018.11.23
			Если Не ЗначениеЗаполнено(Стр2.ДокументОприходования) Тогда
				стр2.Себестоимость = 0;
			КонецЕсли;
		...
		Если Не Проводить тогда 
			Попытка 
				...
				// + Граховский К.Н. 2018.11.08
				РегистрыСведений._СоответствияОбъектовАйко.ДобавитьСоответствие("ТребованиеНакладная", докСтр.Номер, Док.Ссылка, докСтр.Номер + " от " + докСтр.Дата);
				// - Граховский К.Н. 2018.11.08
...
Процедура СоздатьПереработки(табз,префикс,ДеньПрефикс,порядок, БазовыйДокумент)
	...
	Для каждого докСтр из Доки цикл
		...
		Иначе
			...
			д = Объект.Докум.НайтиСТроки(Новый Структура("Документ", базовыйДокумент));
			Если д.Количество() = 0 или не д[0].Выбор тогда
				Продолжить;
			КонецЕсли;
		КонецЕсли;
		...
		Если НЕ Проводить тогда
			Попытка 
				...
				Если ПустаяСтрока(базовыйДокумент) Тогда
					Объект.Докум.НайтиСтроки(Новый Структура("Документ", номернаклиико))[0].Док=Док.Ссылка;
				Иначе
					Объект.Докум.НайтиСтроки(Новый Структура("Документ", базовыйДокумент))[0].Док=Док.Ссылка;	
					НужнаКолонка = Истина;
				КонецЕсли;
...
Процедура СоздатьПКВ(табз,префикс,порядок, тз1, ТЗ10)	
	...
	Если ТЗ10 <> Неопределено тогда
		...
		Для каждого Стр из ДокиАУ цикл
			...
			Иначе
				Сообщить("Невозможно распределить безналичную выручку");
				Прервать;
			КонецЕсли;
	...
	Для каждого стр из Доки2 цикл
		...
		Для каждого строка из Нстр1 цикл			
			...
			Если (строка.Видоплатынаименование = "Наличные") или (строка.Видоплатынаименование = "Рубли") тогда 
				Продолжить;
			Иначе  		
						...
						Пока СуммаБезнала>0 цикл
							...
							Если Д.Количество() > 0 тогда 
								...
								Иначе
									...
									Прервать;
								КонецЕсли;
								Если (Док.Товары.Итог("Сумма")- Док.Оплата.Итог("СуммаОплаты"))>=СуммаБезнала тогда								
									...
									Док.Записать(РежимЗаписиДокумента.Запись);
									СуммаБезнала = 0;
								ИначеЕсли (Док.Товары.Итог("Сумма")- Док.Оплата.Итог("СуммаОплаты")) < Строка.Сумма тогда 
									ххх = Док.Оплата.Добавить();
									ххх.ВидОплаты = Во;
...
Функция СохранитьСклады() 
	...
	Для каждого Стр из Объект.Склады цикл
		...
		Иначе
			...
			// + Граховский К.Н. 2018.11.12
			РегистрыСведений._СоответствияОбъектовАйко.ДобавитьСоответствие("Склады", Стр.СкладКод, Стр.Склад, Стр.СкладНаименование);
			РегистрыСведений._СоответствияОбъектовАйко.ДобавитьСоответствие("ПодразделенияОрганизаций", Стр.СкладКод, Стр.Подразделение, Стр.СкладНаименование);
			// - Граховский К.Н. 2018.11.12
...
Функция СохранитьНоменклатуру() 
	...
	Для каждого Стр из Объект.Номенклатура цикл
		...
		// + Граховский К.Н. 2018.11.12
		РегистрыСведений._СоответствияОбъектовАйко.ДобавитьСоответствие("Номенклатура", кодАйко, Стр.Номенклатура, Стр.ННаименованиеИИКО);
		// - Граховский К.Н. 2018.11.12
...
Процедура СохранениеНастроек(ТекущаяНастройка)
	...
	// + Граховский К.Н. 2018.09.17 дубль
	//УстановитьПривилегированныйРежим(Истина);
	// - Граховский К.Н. 2018.09.17
...
Функция СохранитьКонтрагентов() 
	...
	Для каждого стр из Объект.Контрагенты цикл
		...
		Иначе
			...
			// + Граховский К.Н. 2018.11.12
			РегистрыСведений._СоответствияОбъектовАйко.ДобавитьСоответствие("Контрагенты", Стр.ККодИИКО, Стр.Контрагент, Стр.КНаименованиеИИКО);
			// - Граховский К.Н. 2018.11.12
...
Процедура СоздатьАПУ (табз,префикс,ДеньПрефикс,порядок)
	...
	Доки.Свернуть("Номер,Дата,ТорговоеПредприятиеКод,ЮЛнаименование,ЮЛИНН,Контрагент,СчетФактура,ВходНомер,ВходДата,Сотрудник,Комментарий");
	 ...
	 Для каждого докСтр из Доки цикл
		...
		// + Граховский К.Н. 2018.11.09
		Док = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ПоступлениеТоваровУслуг", докСтр.Номер);
		// - Граховский К.Н. 2018.11.09
		Если не ЗначениеЗаполнено(Док) Тогда
			Док = Документы.ПоступлениеТоваровУслуг.СоздатьДокумент();
			// + Граховский К.Н. 2018.11.08
...
Функция СохранитьСтатьи() 
	...
	Для каждого Стр из Объект.Статьи цикл
		Если не ЗначениеЗаполнено(Стр.СтатьяЗатрат1С) Тогда
			Сообщить("Для загруженной статьи расходов " + Стр.СкладНаименование +  " не сопоставлена статья затрат в 1С", СтатусСообщения.ОченьВажное);
			Ошибок = Ошибок  + 1;
		Иначе
...
Процедура СоздатьПриготовлениеСписания(таб,префикс,ДеньПрефикс,порядок, сдвинутьДату,базовыйДокумент)
	...
	Для каждого стр из Доки цикл
		...
		Док1 = Документы.ПеремещениеТоваров.НайтиПоНомеру(НомерДокумента,ДатаДокум);
		// + Граховский К.Н. 2018.11.09
		Док1 = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ПеремещениеТоваров", Стр.Номер); 
		// - Граховский К.Н. 2018.11.09
		Если не ЗначениеЗаполнено(Док1) Тогда
			Док1 = Документы.ПеремещениеТоваров.СоздатьДокумент();
			// + Граховский К.Н. 2018.11.09
		...
		// + Граховский К.Н. 2018.11.09
		Если Док1.ЭтоНовый() Тогда
			Док1.УстановитьНовыйНомер();
		КонецЕсли;
		Док1.Комментарий = "Загружено из iiko " + Стр.Номер;
		// - Граховский К.Н. 2018.11.09
		...
		Если Проводить тогда 			
			Попытка 
				...
				КонецЕсли;
				// + Граховский К.Н. 2018.11.08
				РегистрыСведений._СоответствияОбъектовАйко.ДобавитьСоответствие("ПеремещениеТоваров", Стр.Номер, Док1.Ссылка, Стр.Номер + " от " + Стр.Дата);
				// - Граховский К.Н. 2018.11.08
			Исключение
				Сообщить("Не удалось провести перемещение товаров № "+Док1.Номер+" от "+Док1.Дата+" "+ОписаниеОшибки(), СтатусСообщения.ОченьВажное);			
			КонецПопытки;
...
// + Граховский К.Н. 2018.11.09
&НаСервере
Процедура ЗаполнитьСоответствияПоРегистру()
	
	//ЗаполняемТаблицу Докум
	Для Каждого СтрокаДокумент Из Объект.Докум Цикл
		
		Если СтрокаДокумент.ВидДок = "Приходная Накладная" Тогда
			СтрокаДокумент.Док = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ПоступлениеТоваровУслуг", СтрокаДокумент.НомерДок);
		КонецЕсли;
		
		Если СтрокаДокумент.ВидДок = "Акт Реализации" Тогда
			СтрокаДокумент.Док = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ОтчетОРозничныхПродажах", СтрокаДокумент.НомерДок);
			СтрокаДокумент.Док2 = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ОтчетПроизводстваЗаСмену", СтрокаДокумент.НомерДок);
			Элементы.ДокумДок2.Видимость = Истина;
		КонецЕсли;
		
		Если СтрокаДокумент.ВидДок = "Акт Списания" Тогда
			СтрокаДокумент.Док = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ТребованиеНакладная", СтрокаДокумент.НомерДок);
		КонецЕсли;
		
		Если СтрокаДокумент.ВидДок = "Внутреннее Перемещение" Тогда
			СтрокаДокумент.Док = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ПеремещениеТоваров", СтрокаДокумент.НомерДок);
			СтрокаДокумент.Док2 = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ТребованиеНакладная", СтрокаДокумент.НомерДок);
			Элементы.ДокумДок2.Видимость = Истина;
		КонецЕсли;
	...
	Для Каждого СтрокаНоменклатура Из Объект.Номенклатура Цикл
		...
		НайденнаяНоменклатура = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("Номенклатура", СтрокаНоменклатура.НКодИИКО);
		Если ЗначениеЗаполнено(НайденнаяНоменклатура) Тогда
			СтрокаНоменклатура.Номенклатура = НайденнаяНоменклатура;
			СтрокаНоменклатура.Наименование1С = НайденнаяНоменклатура.Наименование;
		КонецЕсли;
	...
	Для Каждого СтрокаСклад Из Объект.Склады Цикл
		...
		НайденныйСклад = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("Склады", СтрокаСклад.СкладКод);
		Если ЗначениеЗаполнено(НайденныйСклад) Тогда
			СтрокаСклад.Склад = НайденныйСклад;
			СтрокаСклад.Наименование1с = НайденныйСклад.Наименование;
		КонецЕсли;
		
		НайденноеПодразделение = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("ПодразделенияОрганизаций", СтрокаСклад.СкладКод);
		Если ЗначениеЗаполнено(НайденноеПодразделение) Тогда
			СтрокаСклад.Подразделение = Подразделение;
		КонецЕсли;
	...
	Для Каждого СтрокаКонтрагент Из Объект.Контрагенты Цикл
		...
		НайденныйКонтрагент = РегистрыСведений._СоответствияОбъектовАйко.НайтиСоответствие("Контрагенты", СтрокаКонтрагент.ККодИИКО);
		Если ЗначениеЗаполнено(НайденныйКонтрагент) Тогда
			СтрокаКонтрагент.Контрагент = НайденныйКонтрагент;
			СтрокаКонтрагент.Наименование1С = НайденныйКонтрагент.Наименование;
			СтрокаКонтрагент.Код1С = НайденныйКонтрагент.Код;
			СтрокаКонтрагент.ИНН1С = НайденныйКонтрагент.ИНН;
		КонецЕсли;
...
&НаСервере
Процедура ДополнительнаяОбработкаПриСозданииНаСервере()
	...
	Элементы.Араз.ЦветТекстаЗаголовка = Новый Цвет(255, 0, 0); // красный
	Элементы.Араз.ШрифтЗаголовка = Новый Шрифт("Arial",10,,,,Истина); 
	Элементы.Поле15.ЦветТекстаЗаголовка = Новый Цвет(255, 0, 0); // красный;
	Элементы.Поле15.ШрифтЗаголовка = Новый Шрифт("Arial",10,,,,Истина);
	Элементы.АП.ЦветТекстаЗаголовка = Новый Цвет(255, 0, 0); // красный
	Элементы.АП.ШрифтЗаголовка = Новый Шрифт("Arial",10,,,,Истина);
	Элементы.АПЕР.ЦветТекстаЗаголовка = Новый Цвет(255, 0, 0); // красный
	Элементы.АПЕР.ШрифтЗаголовка = Новый Шрифт("Arial",10,,,,Истина);
	Элементы.ИН.ЦветТекстаЗаголовка = Новый Цвет(255, 0, 0); // красный
	Элементы.ИН.ШрифтЗаголовка = Новый Шрифт("Arial",10,,,,Истина);
	Элементы.Поле19.ЦветТекстаЗаголовка = Новый Цвет(255, 0, 0); // красный
	Элементы.Поле19.ШрифтЗаголовка = Новый Шрифт("Arial",10,,,,Истина);
	Элементы.АПУ.ЦветТекстаЗаголовка = Новый Цвет(255, 0, 0); // красный
	Элементы.АПУ.ШрифтЗаголовка = Новый Шрифт("Arial",10,,,,Истина);
	Элементы.АУ.ЦветТекстаЗаголовка = Новый Цвет(255, 0, 0); // красный
	Элементы.АУ.ШрифтЗаголовка = Новый Шрифт("Arial",10,,,,Истина);
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьСпособУчетаНДСДляСтрокиТабличнойЧасти(СтрокаТабличнойЧасти, ОбрабатываемыйОбъект)
	
	Если ТипЗнч(ОбрабатываемыйОбъект) = Тип("ДокументОбъект.ПоступлениеТоваровУслуг") Тогда
		Склад = ОбрабатываемыйОбъект.Склад;
		СтрокаТабличнойЧасти.СпособУчетаНДС = Перечисления.СпособыУчетаНДС.УчитываетсяВCтоимости;
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(ОбрабатываемыйОбъект) = Тип("ДокументОбъект.ПеремещениеТоваров") Тогда
		Склад = ОбрабатываемыйОбъект.СкладОтправитель;
	КонецЕсли;
	
	Если ТипЗнч(ОбрабатываемыйОбъект) = Тип("ДокументОбъект.ОтчетПроизводстваЗаСмену")
		ИЛИ ТипЗнч(ОбрабатываемыйОбъект) = Тип("ДокументОбъект.ТребованиеНакладная") Тогда
		Склад = ОбрабатываемыйОбъект.Склад;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(СпособУчетаНДС) Тогда
