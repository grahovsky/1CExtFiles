...
Функция ПолучитьИспользуемыеПараметры(Знач ИД_Базы)
	...
	СтрокаПользователиБазы = Пользователи.НайтиСтроки(СтруктураОтбора)[0];
	...
	СтруктураПараметров.Вставить("ИмяПользователяБазы", СтрокаПользователиБазы.Имя);
	СтруктураПараметров.Вставить("ПарольПользователяБазы", СтрокаПользователиБазы.Пароль);
...
Функция ПолучитьСтрокуЗапуска(СтруктураПараметров, ВидЗапуска)
	...
	ИначеЕсли ВидЗапуска = "ВыполнитьОбъединениеКонфигураций" Тогда
...
Функция ПолучитьСписокДействий()
	...
	СписокДействий.Добавить("ВыполнитьОбъединениеКонфигураций");
...
Функция ОбработатьДействие(ВыполняемоеДействие)
	...
	Если ВыполняемоеДействие.Действие = "ВыполнитьОбъединениеКонфигураций" Тогда
		
		Если ВыполняемоеДействие.Статус = "" Тогда
			
			Результат = РаботаСКластеромСерверов1С(ИнформацияОСистеме);
			Если НЕ Результат.ЕстьСеансКонфигуратора Тогда
				ВыполнитьПакетныйЗапуск(ВыполняемоеДействие.ИД_Базы, "ВыполнитьОбъединениеКонфигураций");
				Лог = Лог + " Запущено ";
				ВыполняемоеДействие.Статус = "Запущено " + ТекущаяДата();
			Иначе
...
Процедура ОбновитьСпискиВыбораЭлементов()
	...
	МассивОбновлений = ПараметрыОбновлений.Выгрузить().ВыгрузитьКолонку("Обновление");
