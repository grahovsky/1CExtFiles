...
Процедура СформироватьПоступление(СтрокаТаблицы, ТаблицаДокументов)
	...
	Для каждого СтрокаТоваровТекДокумента Из ТоварыТекущегоДокумента Цикл
		...
		//////НоваяСтрокаДокумента.СтавкаНДС = ПолучитьСтавкуНДС(СтрокаТоваровТекДокумента.НДС);
		//////
		////////
...
Процедура СформироватьРеализацию()
	...
	Для каждого СтрокаДерева Из рдДеревоДокументовРеализацииОбъект.Строки Цикл
		...
		Для каждого СтрокаУслуг Из УслугиТекущегоДокумента Цикл
			...
			Если СтрокаУслуг.СтавкаНДС < 5 Тогда
				НоваяСтрокаДокумента.СтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
			ИначеЕсли СтрокаУслуг.СтавкаНДС < 12 Тогда	
				НоваяСтрокаДокумента.СтавкаНДС = Перечисления.СтавкиНДС.НДС10;
			Иначе
				НоваяСтрокаДокумента.СтавкаНДС = Перечисления.СтавкиНДС.НДС18;	
			КонецЕсли; 
...
Процедура СформироватьДокументыСтраховыхКомпанийНаСервере()
	...
	Для каждого СтрокаДерева_Услуги Из рдДеревоДляСтраховыхКомпанийОбъект.Строки Цикл
		...
		Если СтрокаТаблицыДокументовРеализации2.СтавкаНДС < 5 Тогда
			НоваяСтрокаДокумента.СтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
		ИначеЕсли СтрокаТаблицыДокументовРеализации2.СтавкаНДС < 12 Тогда	
			НоваяСтрокаДокумента.СтавкаНДС = Перечисления.СтавкиНДС.НДС10;
		Иначе
			НоваяСтрокаДокумента.СтавкаНДС = Перечисления.СтавкиНДС.НДС18;	
		КонецЕсли; 
