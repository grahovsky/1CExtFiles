...
Функция ПолучитьСтрокуЗапуска(СтруктураПараметров, ВидЗапуска)
	...
	ИначеЕсли ВидЗапуска = "ОтключитьсяОтХранилища" Тогда
		ПараметрыВыполнения = "/ConfigurationRepositoryUnbindCfg -force";	
	ИначеЕсли ВидЗапуска = "ПодключитьсяКХранилищу" Тогда
...
Функция ПолучитьСписокДействий()
	...
	СписокДействий.Добавить("ОтключитьсяОтХранилища");
	СписокДействий.Добавить("ПодключитьсяКХранилищу");
...
Функция ОбработатьДействие(ВыполняемоеДействие)
	...
	Если ВыполняемоеДействие.Действие = "ОтключитьсяОтХранилища" Тогда
		
		Если ВыполняемоеДействие.Статус = "" Тогда
			
			Результат = РаботаСКластеромСерверов1С(ИнформацияОСистеме);
			Если НЕ Результат.ЕстьСеансКонфигуратора Тогда
				ВыполнитьПакетныйЗапуск(ВыполняемоеДействие.ИД_Базы, "ОтключитьсяОтХранилища");
				Лог = Лог + " Запущено ";
				ВыполняемоеДействие.Статус = "Запущено " + ТекущаяДата();
			Иначе
				Лог = Лог + " нет возможности отключения от хранилища, сеанс конфигуратора";
			КонецЕсли;
			ЗаписатьЛог(Лог, ИнформацияОСистеме.ИмяФайлаЛогов);
	...
	Если ВыполняемоеДействие.Действие = "ПодключитьсяКХранилищу" Тогда
		
		Если ВыполняемоеДействие.Статус = "" Тогда
			
			Результат = РаботаСКластеромСерверов1С(ИнформацияОСистеме);
			Если НЕ Результат.ЕстьСеансКонфигуратора Тогда
				ВыполнитьПакетныйЗапуск(ВыполняемоеДействие.ИД_Базы, "ПодключитьсяКХранилищу");
				Лог = Лог + " Запущено ";
				ВыполняемоеДействие.Статус = "Запущено " + ТекущаяДата();
			Иначе
				Лог = Лог + " нет возможности подключиться к хранилищу, сеанс конфигуратора";
			КонецЕсли;
			ЗаписатьЛог(Лог, ИнформацияОСистеме.ИмяФайлаЛогов);
	...
	Если ВыполняемоеДействие.Действие = "ОбновитьКонфигурациюИзХранилища" Тогда
		...
		Если ВыполняемоеДействие.Статус = "" Тогда
			...
			Если НЕ Результат.ЕстьСеансКонфигуратора Тогда
				...
				ВыполняемоеДействие.Статус = "Запущено " + ТекущаяДата();
			Иначе
				Лог = Лог + " нет возможности обновления из хранилища, сеанс конфигуратора";
			КонецЕсли;
			ЗаписатьЛог(Лог, ИнформацияОСистеме.ИмяФайлаЛогов);
